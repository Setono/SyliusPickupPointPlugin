    ...
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($container->findTaggedServiceIds('setono_sylius_pickup_point.provider') as $id => $tagged) {
            foreach ($tagged as $attributes) {
                if (!is_array($attributes) || !isset($attributes['code'], $attributes['label']) || !is_string($attributes['code']) || !is_string($attributes['label'])) {
                    throw new InvalidArgumentException('Tagged pickup point provider `' . $id . '` needs to have `code` and `label` string attributes.');
                }
